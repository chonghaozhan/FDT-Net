...
'''
This is the model of FDT-Net of HFT stage.
'''
...
class Pre(torch.nn.Module):  # HFEM（High Feature Extract Module）
    def __init__(self, inchannels,size_h,size_w):
        super().__init__()
        self.dct=MultiSpectralAttentionLayer(36,size_h,size_w)
    ...
    def forward(self, inputs):
        ...
        Yh11=Yh[0][:,:,4,:,:,1]
        Yh12=Yh[0][:,:,5,:,:,1]
        Yh13=torch.cat([Yh1,Yh2,Yh3,Yh4,Yh5,Yh6,Yh7,Yh8,Yh9,Yh10,Yh11,Yh12],dim=1) # torch.Size([4, 3, 160, 160])*12  -> torch.Size([4, 36, 160, 160])
        Yh13=torch.nn.functional.interpolate(Yh13, scale_factor=2) # torch.Size([4, 36, 320, 320])
        Yh13=self.dct(Yh13)  # torch.Size([4, 36, 320, 320])

        Yh1,Yh2,Yh3,Yh4,Yh5,Yh6,Yh7,Yh8,Yh9,Yh10,Yh11,Yh12=Yh13.split(3,dim=1) # torch.Size([4, 3, 320, 320]) *12   
        Yh1=torch.sum(Yh1,dim=1,keepdim=True)   # torch.Size([4,1,320,320])
        Yh2=torch.sum(Yh2,dim=1,keepdim=True)  
        Yh3=torch.sum(Yh3,dim=1,keepdim=True)
        Yh4=torch.sum(Yh4,dim=1,keepdim=True)
        Yh5=torch.sum(Yh5,dim=1,keepdim=True)
        Yh6=torch.sum(Yh6,dim=1,keepdim=True)
        Yh7=torch.sum(Yh7,dim=1,keepdim=True)
        Yh8=torch.sum(Yh8,dim=1,keepdim=True)
        Yh9=torch.sum(Yh9,dim=1,keepdim=True)
        Yh10=torch.sum(Yh10,dim=1,keepdim=True)
        Yh11=torch.sum(Yh11,dim=1,keepdim=True)
        Yh12=torch.sum(Yh12,dim=1,keepdim=True)
        Yh13=torch.cat([Yh1,Yh2,Yh3,Yh4,Yh5,Yh6,Yh7,Yh8,Yh9,Yh10,Yh11,Yh12],dim=1) # torch.Size([4, 12, 320, 320])
        Yh13=self.preconv(Yh13) # torch.Size([4, 3, 320, 320])
        return Yh13 
...
class Networkv2(nn.Module):
    def __init__(self):
        ...
        self.res2net=res2net50(pretrained=True).to('cuda')
        for p in self.parameters(): 
            p.requires_grad=False
        # frozen the parameters begore
        self.pre=Pre(3,320,320)
        self.se4=se_block(1024+256,256) #
        self.se3=se_block(512+128,128)
        self.se2=se_block(256+64,64)
    ...
    def forward(self, inp):
        ...
        inp_high=self.pre(inp) # 3,320
        inp_high_80,inp_high_40,inp_high_20=self.res2net(inp_high)
        
        
        
        inc = self.inception(inp)  # 3 320
        input = self.input(inc)  # 16 320
        # calayer = self.calayer(input)+input  #calayer torch.Size([8, 16, 320, 320]) ------

        #env1 = self.enc1(calayer)  # 32 160  env1 torch.Size([8, 32, 160, 160])
        #env2 = self.enc2(env1)  # 64 80
        #env3 = self.enc3(env2)  # 128 40
        #env4 = self.enc4(env3)  # 256 20
        ...
        mid=self.se4(torch.cat((mid, inp_high_20), dim=1))
        ...
        dev4=self.se3(torch.cat((dev4, inp_high_40), dim=1))
        dev3 = self.dec3((dev4, env2, env1))  # 128->64 80
        
        dev3=self.se2(torch.cat((dev3, inp_high_80), dim=1))
